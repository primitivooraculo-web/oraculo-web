...
const { handleGarageVaultAction } = require("./garage-vault");
const { buildOraculoCapabilities } = require("./oraculo-capabilities");
const { adminSkinQueue, handleSkinEditor, handleSkinMod, skinConfig } = require("./skins");
...
const SKIN_MOD_TOKEN = process.env.ORACULO_SKIN_MOD_TOKEN || "";
...
const COMMAND_TEMPLATES = {
  ...
  "restore-dino": process.env.ADMIN_AGENT_CMD_RESTORE_DINO || "",
...
function getToken(req) {
  ...
  return req.headers["x-oraculo-agent-token"] || req.headers["x-oraculo-mod-token"] || "";
}
...
function requireSkinMod(req, res) {
  if (!SKIN_MOD_TOKEN) {
    sendJson(res, 503, {
      success: false,
      error: "Falta ORACULO_SKIN_MOD_TOKEN.",
    });
    return false;
  }

  if (getToken(req) !== SKIN_MOD_TOKEN) {
    sendJson(res, 401, {
      success: false,
      error: "Token de mod invalido.",
    });
    return false;
  }
...
function capabilities() {
  const currentSkinConfig = skinConfig();
  return {
    success: true,
    ...
    garageVault: {
      ...
      requires: {
        save: "ADMIN_AGENT_ACTIONS_ENABLED=true + ADMIN_AGENT_CMD_KILL_DINO",
        reuse: "ADMIN_AGENT_ACTIONS_ENABLED=true + ADMIN_AGENT_CMD_RESTORE_DINO",
      },
    },
    skins: {
      active: [
      ...
      store: "local-json",
      liveApply: Boolean(SKIN_MOD_TOKEN),
      skinMod: Boolean(SKIN_MOD_TOKEN),
      config: currentSkinConfig,
    },
    oraculo: buildOraculoCapabilities({
      actionsEnabled: ACTIONS_ENABLED,
      allowConsole: ALLOW_CONSOLE,
      commandTemplates: COMMAND_TEMPLATES,
      skinModToken: SKIN_MOD_TOKEN,
      skinConfig: currentSkinConfig,
      standardRconActions: STANDARD_RCON_ACTIONS,
    }),
  };
}
...
function garageVaultDeps() {
  return {
    getPlayerData,
    killDino: (body) => handleAdvancedAction("kill-dino", body),
    restoreDino: (body) => handleAdvancedAction("restore-dino", body),
  };
}
...
async function handleRequest(req, res) {
  ...
  try {
    ...
    if (url.pathname.startsWith("/api/mod/skins/") && !requireSkinMod(req, res)) {
      return;
    }
    ...
    if (req.method === "GET" && url.pathname === "/api/admin/oraculo-capabilities") {
      return sendJson(res, 200, capabilities().oraculo);
    }

    if (url.pathname.startsWith("/api/mod/skins/")) {
      const body = req.method === "GET" ? {} : await readBody(req);
      const params = Object.fromEntries(url.searchParams.entries());
      return sendJson(res, 200, await handleSkinMod(url.pathname, req.method, params, body));
    }
    ...
    if (req.method === "GET" && url.pathname === "/api/admin/skins") {
      const params = Object.fromEntries(url.searchParams.entries());
      return sendJson(res, 200, adminSkinQueue(params));
    }

    if (req.method === "POST" && url.pathname === "/api/admin/skins") {
      const body = await readBody(req);
      return sendJson(res, 200, await handleSkinMod("/api/mod/skins/ack", "POST", {}, body));
    }

    if (req.method === "GET" && url.pathname === "/api/admin/garage-vault") {
      const body = Object.fromEntries(url.searchParams.entries());
      const op = url.searchParams.get("op") || "list";
      return sendJson(res, 200, await handleGarageVaultAction(op, body, garageVaultDeps()));
    }

    if (req.method === "POST" && url.pathname === "/api/admin/garage-vault") {
      const body = await readBody(req);
      return sendJson(
        res,
        200,
        await handleGarageVaultAction(requiredText(body.action || "save-active", "action"), body, garageVaultDeps()),
      );
    }
